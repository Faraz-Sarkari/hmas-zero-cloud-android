...
def termux_notifier(title, message, urgent=False):
    ...
    if urgent:
        for _ in range(10):
            subprocess.Popen(["termux-notification", "--id", "999", "--title", title, "--content", message, "--priority", "high", "--vibrate", "1000,500,1000", "--ongoing"])
            if contact:
...
def on_results(results):
    ...
    msg = "\n".join(lines)
    termux_notifier(title=f"{'🚨 ' if urgent else ''}{ item_label} Alert", message=msg, urgent=urgent)
    print(msg)
...
if __name__ == "__main__":
    ...
    agent = DataExtractionAgent(plugin_config=config, plugin_dir=PLUGIN_DIR, on_results=on_results)
    agent.start()
